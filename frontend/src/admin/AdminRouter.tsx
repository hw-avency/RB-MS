...
type Tenant = { id: string; domain: string; name?: string | null; entraId?: string | null; employeeCount?: number; createdAt?: string; updatedAt?: string };
    ...
    <AdminLayout path={path} navigate={navigate} title="Mandanten" onLogout={onLogout} currentUser={currentUser ?? null}>
      <section className="card stack-sm">
        <ListToolbar
          ...
          actions={<button className="btn" onClick={() => { setCreating(true); setEditing(null); setDomain(''); setName(''); setEntraId(''); }}>Neuer Mandant</button>}
        />
        {state.error && <ErrorState text={state.error} onRetry={() => void load()} />}
        <div className="table-wrap"><table className="admin-table"><thead><tr><th>Domain</th><th>Name</th><th>Entra ID</th><th>Mitarbeiter</th><th className="align-right">Aktionen</th></tr></thead>{state.loading && !state.ready ? <SkeletonRows columns={5} /> : <tbody>{tenants.map((tenant) => <tr key={tenant.id}><td>{tenant.domain}</td><td>{tenant.name ?? '—'}</td><td className="truncate-cell" title={tenant.entraId ?? ''}>{tenant.entraId ?? '—'}</td><td>{tenant.employeeCount ?? '—'}</td><td className="align-right"><RowMenu items={[{ label: 'Bearbeiten', onSelect: () => { setEditing(tenant); setCreating(true); setDomain(tenant.domain); setName(tenant.name ?? ''); setEntraId(tenant.entraId ?? ''); } }, { label: 'Löschen', danger: true, onSelect: async () => { try { await del(`/admin/tenants/${tenant.id}`); toasts.success('Mandant gelöscht'); await load(); } catch (error) { toasts.error(error instanceof Error ? error.message : 'Löschen fehlgeschlagen'); } } }]} /></td></tr>)}</tbody>}</table></div>
      </section>
      {creating && (
        <div className="overlay"><section className="card dialog stack-sm"><h3>{editing ? 'Mandant bearbeiten' : 'Mandant anlegen'}</h3><form className="stack-sm" onSubmit={async (event) => { event.preventDefault(); try { if (editing) { await patch(`/admin/tenants/${editing.id}`, { domain: normalizedDomain, name, entraId: entraId.trim() || null }); } else { await post('/admin/tenants', { domain: normalizedDomain, name, entraId: entraId.trim() || null }); } setCreating(false); setEditing(null); setDomain(''); setName(''); setEntraId(''); toasts.success('Mandant gespeichert'); await load(); } catch (error) { toasts.error(error instanceof Error ? error.message : 'Speichern fehlgeschlagen'); } }}><label className="field"><span>Domain</span><input required value={domain} onChange={(event) => setDomain(event.target.value)} placeholder="avency.de" /></label><label className="field"><span>Name</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder="optional" /></label><label className="field"><span>Microsoft Entra Tenant ID</span><input value={entraId} onChange={(event) => setEntraId(event.target.value)} placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" /><small style={{ color: '#666', fontSize: '0.875rem' }}>Optional: Ermöglicht Login über Microsoft für diesen Mandanten</small></label><div className="inline-end"><button type="button" className="btn btn-outline" onClick={() => setCreating(false)}>Abbrechen</button><button className="btn" disabled={!normalizedDomain || !normalizedDomain.includes('.')}>Speichern</button></div></form></section></div>
      )}
    </AdminLayout>
  );
}
